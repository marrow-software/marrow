...
- **Design system — Signal (#312, from #306/#309/#310)**: `web/app/globals.css` is the single token seam carrying identity, type, and motion. One restrained spruce accent (`#0f766e` light / `#3aa88f` dark) used only for interaction/active/links/focus over a biased neutral (spruce-biased light, cool-slate dark); unified surface (sidebar = background, no darker-sidebar model); flat **6px** radius across the set (no wide ramp). Named tokens: type ramp `--text-2xs…--text-lg` (chrome default `--text-base` 14, editor body `--text-md` 15) + heading ramp `--h1…--h6`, weights ≤660, `--measure` 68ch, 4px spacing `--s1…--s11`, control heights `--ctl-sm/md/lg` (24/28/32); motion `--ease-signature`, `--dur-hover/state/enter`, feedback primitives (`--lift`/`--press`/`--pop-from`/`--focus-ring`), `--shadow-flat` + `--shadow-signature` (layered, per-theme), `--texture-grain` (light-only `feTurbulence` soft-light overlay, ~5%, via `body::before`; dark off). Reduced motion honored via `@media (prefers-reduced-motion: reduce)`. **Fonts** (`web/app/layout.tsx`): Inter (sans, `--font-inter`) + JetBrains Mono (restrained mono voice, `--font-jetbrains-mono`); Fraunces/serif fully retired — no `--font-heading`. **Regression guard**: `web/scripts/check-tokens.mjs` (`npm run test:tokens`, wired into web CI) fails if any retired token (`#9a3412`/`#e8805c`/Fraunces/`--font-heading`/`--color-cream`/`--color-bone`/wide radius ramp) reappears or any Signal signature token goes missing.
