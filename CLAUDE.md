...
│   │   ├── app-sidebar.tsx           # Unified sidebar (#313, #316): workspace switcher (top) → global-nav strip (Home·Search·Inbox·Shared·Spaces) → divider → current-space tree / surfaced panel → account (bottom). Absorbs the old app-rail. Spaces row opens a switcher flyout; the tree browses the current space only. Tree = folders expand/collapse; pages navigate; drag-and-drop, inline create
...
- **Comments**: `useComments(nodeId)` hook (`hooks/use-comments.ts`) owns thread state; `CommentsDrawer` renders threads/composer/resolve and `CommentBubbleFab` shows the unread badge. Unread = comments created after the viewer's last drawer visit, tracked client-side in `localStorage` (`marrow:comment-visit:<nodeId>`) — deliberately simple v1 heuristic, no backend visit table. **Docks + reflows (#314)**: `page-editor.tsx` wraps the editor + drawer in a two-column grid; opening comments animates the second track (`clamp(15rem,26%,26rem)`) over `--dur-enter` so the editor reflows narrower rather than being overlaid; the panel enters with `.signal-enter` (`--pop-from` fade+translate). Reduced motion collapses the reflow duration, layout stays usable.
- **Unified sidebar (#313)**: `WorkspaceShell` renders a single `AppSidebar` column (the old `app-rail.tsx` is gone). The global-nav strip (Home·Search·Inbox·Shared with me) replaces the icon rail; Search/Inbox surface `rail-panels/*` in place of the tree, Home links to `/home`, Shared is an empty-state stub (no backend yet). Selected nav rows use `.signal-nav-active` (static 2.5px accent bar + soft fill). The workspace switcher (top) and account menu (bottom) absorb the rail's user menu + the old `settings-dialog.tsx` (theme toggle + sign out); export (a self-contained `ExportDialog` menu-item trigger) / org-settings live in the switcher menu. Starred pages moved to Home (the sidebar Starred panel was retired). `WorkspaceShell` toggles the whole sidebar with ⌘/Ctrl+B; when collapsed a floating `PanelLeftOpen` button (top-left of the main column) reopens it.
- **Spaces switcher flyout (#316)**: splits switching spaces from browsing one. A **Spaces** row in the nav strip (below Shared, above a divider) opens a floating switcher flyout (`.signal-enter` — `--pop-from` + fade over `--dur-enter`) anchored beside the row — no scrim/backdrop blur, dismiss on outside-click/Escape. Flyout = filter box, **Current** space, all other spaces, then **View all spaces** (→ `/orgs/{orgId}/admin?section=spaces&ws={workspaceId}`) / **Create a space** (inline `createSpace`) / **Import from other tools** (disabled Phase-1 stub — no space-star backend yet, so the prototype's "Starred" section lists all spaces instead). The inline tree below the divider browses the **current space only**, its name a plain label (no accent). Current space = the route's space, else the last-picked space (persisted in `localStorage` as `marrow.space.current.<userId>.<workspaceId>`), else the first. Nav-strip, tree, and flyout rows all render at the shared `--text-base` scale via `navRowClass()` / `flyoutRowClass()`; `useOutsideDismiss()` is the single dismissal scaffold.
- **Settings replaces the tree (#317)**: the org admin surface (`app/orgs/[orgId]/admin/layout.tsx`) is the unified single-column settings sidebar (shell-ia prototype B `sidebarB_settings`), not the old two-column screen — a **"Back to [workspace]"** row above the settings menu, the menu at the shared `--text-base` scale with Signal tokens (`signal-flat`/`signal-focus`/`signal-nav-active`), a bottom account row, and a **full-width** `main`. Entry points thread the origin workspace via **`?ws=`** so the Back row returns to the exact space tree (`/w/{ws}`), falling back to **Home** (`/home`, "Back to Home") when `ws` is absent or unresolvable: the Spaces flyout's "View all spaces", the workspace switcher's "Organization settings" (→ `/orgs/{orgId}/settings?ws=`), and the settings page's "Admin dashboard" link. Section links preserve `ws`. The `initials()` avatar helper is shared from `lib/utils.ts` (used here + `app-sidebar.tsx`). Owner-gate unchanged (OIDC session → owner-only). Phase-1 deviation: the prototype's top workspace-switcher is omitted (the route is org-scoped) and this is a route-level facsimile of the shell, not a state swap inside `WorkspaceShell`.
- **Inbox**: `rail-panels/inbox-panel.tsx` lists notifications with kind-specific icons/copy and an empty state; `WorkspaceShell` fetches the unread count on mount and the sidebar nav strip renders an unread badge on the Inbox row. Backend delivery lives in `api/marrow/notifications.py` — `@`-mention saves on page nodes notify newly-mentioned users (only mentions new vs. the prior revision; the actor is never self-notified). Notifications are user-scoped and deliberately excluded from export/restore.
...
- **Design system — Signal (#312, from #306/#309/#310)**: `web/app/globals.css` is the single token seam carrying identity, type, and motion. One restrained spruce accent (`#0f766e` light / `#3aa88f` dark) used only for interaction/active/links/focus over a biased neutral (spruce-biased light, cool-slate dark); unified surface (sidebar = background, no darker-sidebar model); flat **6px** radius across the set (no wide ramp). Named tokens: type ramp `--text-2xs…--text-lg` (chrome default `--text-base` 14, editor body `--text-md` 15) + heading ramp `--h1…--h6`, weights ≤660, `--measure` 68ch, 4px spacing `--s1…--s11`, control heights `--ctl-sm/md/lg` (24/28/32); motion `--ease-signature`, `--dur-hover/state/enter`, feedback primitives (`--lift`/`--press`/`--pop-from`/`--focus-ring`), `--shadow-flat` + `--shadow-signature` (layered, per-theme), `--texture-grain` (light-only `feTurbulence` soft-light overlay, ~5%, via `body::before`; dark off). Reduced motion honored via `@media (prefers-reduced-motion: reduce)`. The text scale lives in a Tailwind `@theme` block so `text-2xs…text-lg` utilities resolve to the Signal ramp (chrome `text-base` = 14px) app-wide while the `--text-*` vars still emit to `:root`. **Feedback utilities (#315)** in `globals.css`: `.signal-raised` (flat-rest → signature-shadow hover step, 1px `--press` on active), `.signal-flat` (colour tween + press for ghost/link), `@utility signal-focus` (instant 3px `--ring` outline, never animated — a real utility so it beats base `outline-none`), `.signal-enter` (`@keyframes signal-pop` fade + `--pop-from` translate for overlays), `.signal-nav-active` (accent bar + soft fill). Wired onto `ui/{button,input,badge}` and the shell/comments overlays. **Fonts** (`web/app/layout.tsx`): Inter (sans, `--font-inter`) + JetBrains Mono (restrained mono voice, `--font-jetbrains-mono`); Fraunces/serif fully retired — no `--font-heading`. **Regression guard**: `web/scripts/check-tokens.mjs` (`npm run test:tokens`, wired into web CI) fails if any retired token (`#9a3412`/`#e8805c`/Fraunces/`--font-heading`/`--color-cream`/`--color-bone`/wide radius ramp) reappears or any Signal signature token goes missing.
