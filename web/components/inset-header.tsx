...
function Breadcrumbs({ nodeId, pageTitle }: { nodeId: string; pageTitle: string }) {
  ...
  const crumb = tree ? findBreadcrumb(tree, nodeId) : null;
  const parts = crumb
    ? [crumb.spaceName, ...crumb.nodePath.slice(0, -1), pageTitle]
    : [pageTitle];
...
export function EditorHeader({
  ...
  return (
    <header className="flex items-center gap-3 border-b border-border px-6 py-3">
      <Breadcrumbs nodeId={nodeId || collectionId || ""} pageTitle={pageTitle} />
